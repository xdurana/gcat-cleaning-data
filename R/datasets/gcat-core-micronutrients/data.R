...
variables %>% as.data.frame %>% write.xlsx('output/datasets/gcat-core/variables.xlsx')

core_data_variables <- core_data %>%
  select(one_of(c('id', 'entity_id', variables$name))) %>%
  unique()

core_data_variables %>% write_csv('output/datasets/gcat-core/data.csv', na = "")
core_data_variables %>% as.data.frame %>% write.xlsx('output/datasets/gcat-core/data.xlsx')
